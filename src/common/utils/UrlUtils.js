export const UrlUtils = {
   ...
   convertUrlPatternToRegExp: function ( pattern ) {
   ...
   createUrlPatterns: function(url){
      var indexProtocolPrefix = url.indexOf("//")
      var protocolPrefix = ""
      var domainUrl = url
      if(indexProtocolPrefix!=-1){
         protocolPrefix = url.substring(0, indexProtocolPrefix+2)
         domainUrl = url.substring(indexProtocolPrefix+2)
      }
      var index = domainUrl.indexOf("/")
      var result = new Array()
      while(index!=-1){
         result.push(protocolPrefix + domainUrl.substring(0,index+1) + "*")
         index = domainUrl.indexOf("/", index+1)
      }
      var index = domainUrl.indexOf("?")
      if(index!=-1){
         result.push(protocolPrefix + domainUrl.substring(0,index+1)+ "*") 
   ...
   getMostLikelyPattern: function(url){
      var urlPattern = url
      var index = url.indexOf("?")
      if(index!=-1){
         urlPattern = url.substring(0, index+1) + "*"
      }
      return urlPattern
   },
   
   urlMatchPattern: function(url, urlPattern){
      var urlRegExp = this.convertUrlPatternToRegExp(urlPattern)
