...
;(function ($) {
  ...
  var Completer = (function () {
    ...
    $.extend(Completer.prototype, {
       ...
       */
      onKeyup: function (e) {
        if(this.lastVal == this.$el.val()){
      	  return;
        }
        this.lastVal = this.$el.val();
        var searchQuery, term;

        searchQuery = this.extractSearchQuery(this.getSearchTerm());
      ...
      getSearchTerm: function(){
      	var textFromHeadToCaret, lastSpaceIndex, lastLineBreakIndex;
      	textFromHeadToCaret = this.getTextFromHeadToCaret();
      	lastSpaceIndex = textFromHeadToCaret.lastIndexOf(' ');
      	lastLineBreakIndex = textFromHeadToCaret.lastIndexOf('\n');
      	if (lastSpaceIndex >= 0 || lastLineBreakIndex >= 0) {
      		return textFromHeadToCaret.substring(Math.max(lastSpaceIndex,lastLineBreakIndex+1));
      	}else{
      		return textFromHeadToCaret;
      	}
  ...
  $.fn.textcomplete = function (strategies) {
    ...
    for(var i=0; i<this.length; i++){
   	 new Completer($(this.get(i)), strategies);
    }
